...
def train_model(df):
    ...
    y = (df['avg_score'] < df['avg_score'].quantile(0.3)).astype(int) # check the purpose of it
...
def analyze(year):
    ...
    df['critical'] = df.apply(
        lambda row: 1 if row['avg_score'] < thresholds['subject_thresholds'][row['subject']] 
                   else 0,
        axis=1
    )
    ...
    if thresholds['use_ml_suggestions'] and df['avg_score'].between(*thresholds['ml_score_range']).any():
        try:
            ...
            try:
                    ...
                    #   print(f"Proba shape: {model.predict_proba(X_pred).shape if hasattr(model, 'predict_proba') else 'N/A'}")
                      ...
                      if probas.shape[1] == 2:  # Binary classification
                            ...
                            confidence = probas[:, 0] if probas.shape[1] == 1 else probas.max(axis=1)
					...
					# Calculate boundary cases (relative to threshold)
                        ...
                        boundary_mask = (df['avg_score'] - threshold).abs() < (0.05 * threshold)  # 5% margin
                        ...
                        ml_insights.update({
    										'feature_importances': dict(zip(['subject', 'avg_score'], model.feature_importances_)),
        											...
        											'cases': df[boundary_mask][['student', 'subject', 'avg_score']]
